...
// ==========================================
(function() {
    const access = sessionStorage.getItem('qr_amiens_auth');
    if (access !== 'ok') {
        const mdp = prompt("Accès restreint - CRM Quiz Room Amiens :");
        if (mdp === "AMIENS2026") { // Tu peux changer ce mot de passe
...
// ==========================================
// 3. LOGIQUE DES RELANCES (Jours Ouvrés)
// ==========================================
...
function getNextRelanceDate(count) {
    const steps = [3, 5, 10]; // Relance à J+3, J+5, J+10...
        ...
        filtered.forEach(c => {
            ...
            card.innerHTML = `
                ...
                <div class="mt-4 flex items-center justify-between">
                    <div class="text-[9px] font-bold text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-full">R : ${c.nbRelances}</div>
...
function copyRelanceScript(id, e) {
    ...
    navigator.clipboard.writeText(texte).then(() => {
        alert("Script de relance copié dans le presse-papier !");
    });
}
